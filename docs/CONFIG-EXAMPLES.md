...
- `promptMaxStickers`：系统提示里常驻的可用表情条数（1~60，控制台「设置 → 表情包」可改）。
  清单一半按使用频次取常用的，一半留给没用过/很久没用的（发掉一张换下一张顶上来）；想让它少用几张就调小，
  调大能让它在更多图里挑，代价是每轮提示词更长。库容量与此无关：收藏表情同步一律拉 500 条，全在库里。
