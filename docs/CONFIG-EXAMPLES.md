...
- 作用：把消息里的语音、音频文件、视频音轨转成文字再交给聊天模型 —— 与模型是否多模态无关。
  控制台「设置 → 聊天设置 → 所有模式 · 语音转文字」里有同一个开关与每小时上限档位。
- **密钥复用**「搜索服务 → 豆包」的 `webSearch.doubao.apiKey`（同一个火山方舟 Agent Plan key）；
  没配 key 时不会把这个工具注入给模型（免得模型调用必失败）。
- **数据流向与计费**：音频会上传到火山方舟做识别，按量计费；`maxPerHour` 是每小时最多转写几次的硬闸门
  （跨会话共享，默认 12，非正数按 12 处理）。不想要这个能力就关掉 `enabled` ——
  它与 `webSearch.enabled` 相互独立，关掉联网搜索不会顺带关掉语音转写，反之亦然。
